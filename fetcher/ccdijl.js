...
const getPagePostArr = (targetDocument) => {
    const postArr = Array.from(
      targetDocument.querySelectorAll(".subnews_list_lj > li > a")
    );
  ...
  const getPageUrl = () => {
    return new Array(9).fill("").map((_, i) => {
      if (i === 0) {
        return "http://www.ccdijl.gov.cn/scdc/djzwcf/index.html";
      }
      return `http://www.ccdijl.gov.cn/scdc/djzwcf/index_${i}.html`;
    });
  };
  ...
  const getPostContentAndTime = async (url) => {
    ...
    const content =
      targetDoc.querySelector(".contentMainPage")?.innerText.trim() || "";
    const publishTime =
      targetDoc
        .querySelector(".pLine")
  ...
  getAllData().then((res) => {
    const data = {
      siteName: "吉林省纪委省监委",
      siteUrl: "http://www.ccdijl.gov.cn/scdc/djzwcf/index.html",
