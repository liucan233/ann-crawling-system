...
const getPagePostArr = (targetDocument) => {
  const postArr = Array.from(
    targetDocument.querySelectorAll(".c-list-post > li > a")
  );
...
const getPageUrl = () => {
  return new Array(11).fill("").map((_, i) => {
    return `https://www.nmgjjjc.gov.cn/category/scdc/zzqjwjwqwfb.html?t=1684395134832#${
      i + 1
    }`;
  });
};
...
const getPostContentAndTime = async (url) => {
  ...
  const content =
    targetDoc.querySelector(".c-content-con")?.innerText.trim() || "";
  const publishTime =
    targetDoc
      .querySelector(".c-pubdate")
      .innerText?.match(/\d+-\d+-\d+(?: \d+:\d+:\d+)?/)?.[0]
      ?.trim() || "";
...
getAllData().then((res) => {
  const data = {
    siteName: "内蒙古纪委监委网站",
    siteUrl:
      "https://www.nmgjjjc.gov.cn/category/scdc/zzqjwjwqwfb.html?t=1684395134832#1",
