...
const siteName='西藏纪检监察网'
const siteUrl='http://www.xzjjw.gov.cn/djzwcf.jhtml'
...
const getPagePostArr = (targetDocument) => {
  const postArr = Array.from(
    targetDocument.querySelectorAll(".list > li > a")
  );
  if (postArr.length < 1) {
...
const getPageUrl = () => {
  return new Array(11).fill("").map((_, i) => {
    ...
    return `http://www.xzjjw.gov.cn/djzwcf_${i+1}.jhtml`;
  });
};
