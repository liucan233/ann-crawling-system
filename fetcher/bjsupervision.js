...
const getPagePostArr = (targetDocument) => {
    const postArr = Array.from(
      targetDocument.querySelectorAll(".list-date02 > li > a")
    );
   ...
   */
  const getPageUrl = () => {
    return new Array(41).fill("").map((_, i) => {
      if (i === 0) {
        return "http://www.bjsupervision.gov.cn/gzzc/zjsc/index.html";
      }
      return `http://www.bjsupervision.gov.cn/gzzc/zjsc/index_${i}.html`;
    });
  };
  ...
  const getPostContentAndTime = async (url) => {
    ...
    const content =
      targetDoc.querySelector(".wz")?.innerText.trim() || "";
    const publishTime =
      targetDoc
        .querySelector(".detail-span")
  ...
  getAllData().then((res) => {
    const data = {
      siteName: "北京纪检监察网",
      siteUrl: "http://www.bjsupervision.gov.cn/gzzc/zjsc/index.html",
