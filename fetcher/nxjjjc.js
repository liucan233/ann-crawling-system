...
const siteName='宁夏纪委监委网'
const siteUrl='http://www.nxjjjc.gov.cn/xbnxjw/xxgk/jlsc/zzqjlsc/index.html'
...
const getPagePostArr = (targetDocument) => {
  const postArr = Array.from(
    targetDocument.querySelectorAll(".news-list > li > a")
  );
  if (postArr.length < 1) {
...
const getPageUrl = () => {
  return new Array(6).fill("").map((_, i) => {
    ...
    return `http://www.nxjjjc.gov.cn/xbnxjw/xxgk/jlsc/zzqjlsc/index_${i}.html`;
  });
};
...
const getPostContentAndTime = async (url) => {
  ...
  const content = targetDoc.querySelector(".news_article")?.innerText.trim() || "";
  const publishTime =
    targetDoc
      .querySelector(".bon")
