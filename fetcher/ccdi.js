...
const getPagePostArr = (targetDocument) => {
  const postArr = Array.from(
    targetDocument.querySelectorAll(".list_news_dl2 > li > div > a")
  );
  if (postArr.length < 1) {
 ...
 */
const getPageUrl = () => {
  return new Array(14).fill("").map((_, i) => {
    if (i === 0) {
      return "https://www.ccdi.gov.cn/scdcn/zggb/djcf/index.html";
    }
    return `https://www.ccdi.gov.cn/scdcn/zggb/djcf/index_${i}.html`;
  });
};
...
const getAllData = async () => {
  ...
  for (let i = 0; i < pageUrlArr.length; i++) {
    ...
    console.log("页面包含文章列表", curPagePostArr);
...
getAllData().then((res) => {
  const data = {
    siteName: "中央纪委国家监委网站",
    siteUrl: "https://www.ccdi.gov.cn/scdcn/zggb/djcf/index.html",
